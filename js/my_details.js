...
let detailsHtmlTags = `<div class="container">
    <div class="row">
    <div>
...
function tableCreation() {
  ...
  for (let i = 0; i < tableData.length; i++) {
    ...
    addElementTagWithHtml(
      tr,
      "td",
      "<button class='btn btn-sm' style='background:red' onclick=updateTableRow('" +
        tableData[i].name +
        "')>update</button>"
    );
    addElementTagWithHtml(
      tr,
      "td",
      "<button class='btn btn-sm' style='background:red' onclick=deleteTableRow('" +
        tableData[i].name +
        "')>delete</button>"
    );
  }
  ...
  for (let i = 0; i < upDateParsedList.length; i++) {
    if (upDateParsedList[i].name === name) {
      ...
      addElementTagButton(
        ...
        "button",
        "onclick",
        "formUpdation('" + upDateParsedList[i].name + "')",
        "Update"
      );
      addElementTagButton(
        myForm,
